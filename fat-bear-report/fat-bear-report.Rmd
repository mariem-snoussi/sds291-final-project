...
## Outcome Variable
The outcome variable for the first hypothesis is number of votes obtained in the first round. This is a quantitative variable in the positive integers, each vote will count as one unit.
...
```{r}
# logit model for SurveyAngle and Win
logitAngle <- glm(CompWinBinary ~ SurveyAngle, data = bears, family = binomial)
summary(logitAngle)
```
...
logitAge <- glm(CompWinBinary ~ ParkAge, data = bears, family = binomial)
summary(logitAge)
```


```{r}
logitAgeAngle <- glm(CompWinBinary ~ ParkAge*SurveyAngle, data = bears, family = binomial)
summary(logitAgeAngle)
```

```{r}
logitAgeAngle2 <- glm(CompWinBinary ~ ParkAge  + SurveyAngle, data = bears, family = binomial)
summary(logitAgeAngle2)
```


Idea for second hypothesis: 
filter for all who won first round. run logitmod with win and number of votes with filtered data
